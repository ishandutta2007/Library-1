...
template <typename M, bool persistent= false, uint8_t HEIGHT= 31> class SegmentTree_Dynamic {
 ...
 template <bool mo_, bool du_, typename tEnable= void> struct Node_D: Node_B<M, Node_D<mo_, du_, tEnable>> {};
 template <bool mo_, bool du_> struct Node_D<mo_, du_, typename std::enable_if_t<mo_ && !du_>>: Node_B<typename M::T, Node_D<mo_, du_>> {};
 template <bool mo_, bool du_> struct Node_D<mo_, du_, typename std::enable_if_t<du_>>: Node_B<typename M::T, Node_D<mo_, du_>, typename M::E> {
  typename M::E lazy;
  bool lazy_flg= false;
 };
 ...
 template <class S> void build(Node *&t, const id_t &n, std::array<id_t, 2> b, const S &bg) {
  if (n <= b[0]) return;
  id_t m= (b[0] + b[1]) >> 1;
  if (b[1] - b[0] == 1) {
   if constexpr (std::is_same_v<S, T>) t= new Node{bg};
 ...
 void dump(Node *t, const id_t &l, const id_t &r, std::array<id_t, 2> b, typename std::vector<T>::iterator itr) {
 ...
 T fold(Node *&t, const id_t &l, const id_t &r, std::array<id_t, 2> b, const id_t &bias) {
 ...
 void apply(Node *&t, const id_t &l, const id_t &r, std::array<id_t, 2> b, const E &x) {
  if (r <= b[0] || b[1] <= l) return;
  id_t m= (b[0] + b[1]) >> 1;
 ...
 template <bool last> static inline T calc_op(Node *&t, const T &v) {
...
public:
 ...
 SegmentTree_Dynamic(std::size_t n, T val): root(nullptr) { build(root, n, {0, 1LL << HEIGHT}, val); }
 SegmentTree_Dynamic(const T *bg, const T *ed): root(nullptr) { build(root, ed - bg, {0, 1LL << HEIGHT}, bg); }
 ...
 static std::string which_available() {
  std::string ret= "";
  if constexpr (monoid<M>::value) ret+= "\"fold\" \"find\" ";
  else ret+= "\"at\" ";
