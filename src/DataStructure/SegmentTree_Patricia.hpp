...
template <typename M, bool persistent= false, std::uint8_t HEIGHT= 30> class SegmentTree_Patricia {
 ...
 template <bool mo_, typename tEnable= void> struct Node_D: Node_B<M, Node_D<mo_, tEnable>> {};
 template <bool mo_> struct Node_D<mo_, typename std::enable_if_t<mo_>>: Node_B<typename M::T, Node_D<mo_>> {};
...
public:
 ...
 SegmentTree_Patricia(std::size_t n, T val): root(nullptr) { build(root, n, {0, 1LL << HEIGHT}, val); }
 SegmentTree_Patricia(T *bg, T *ed): root(nullptr) { build(root, ed - bg, {0, 1LL << HEIGHT}, bg); }
 ...
 template <class L= M, typename std::enable_if_t<monoid<L>::value> *= nullptr> T operator[](id_t k) { return get(k); }
 template <class L= M, typename std::enable_if_t<!monoid<L>::value> *= nullptr> T &operator[](id_t k) { return at(k); }
