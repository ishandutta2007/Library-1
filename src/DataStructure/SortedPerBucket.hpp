...
template <class T, std::size_t B= 700> class SortedPerBucket {
 ...
 struct Dat {
  ...
  std::array<T, B + 1> acc;
  T add, lb, ub;
  Dat(std::size_t b): n(b), a{0}, sorted{0}, acc{0}, add(0), lb(-INF), ub(INF) {}
  Dat(const T *bg, std::size_t b): n(b), a{0}, acc{0}, add(0), lb(-INF), ub(INF) {
  ...
  inline std::size_t idx(T x) const { return std::lower_bound(sorted.begin(), sorted.begin() + n, x) - sorted.begin(); }
  inline std::size_t count(T x) const { return x-= add, (x <= lb ? 0 : ub < x ? n : idx(x)); }
  inline T sum() const {
 ...
 const std::size_t n;
 std::vector<Dat> dat;
 template <class U, class All, class One> inline U fold(std::size_t l, std::size_t r, const All &all, const One &one) const {
  U ret= 0;
 ...
 template <class All, class One> inline void update(std::size_t l, std::size_t r, const All &all, const One &one) {
  if (std::size_t i= l / B, j= r / B, k= l % B, m= r % B; i < j) {
   if (k) {
...
public:
 ...
 std::size_t count(std::size_t l, std::size_t r, T lb, T ub) const { return count(l, r, ub) - count(l, r, lb); }
 // sum a[i] s.t. (l <= i < r)
 ...
 T sum(std::size_t l, std::size_t r, T lb, T ub) const { return sum(l, r, ub) - sum(l, r, lb); }
