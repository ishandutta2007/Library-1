...
struct SubProductTree {
  ...
  SubProductTree(const std::vector<mod_t> &xs_)
      : xs(xs_), n(xs_.size()), nn(get_len(n)), p(nn << 1) {
    xs.resize(nn, Z);
    for (int i = 0; i < nn; ++i)
      p[nn + i].resize(2), p[nn + i].set(0, 1), p[nn + i].set(1, -xs[i]);
    for (int i = nn; --i;) {
      int ls = i << 1, rs = i << 1 | 1, len = get_len(p[ls].size());
      p[ls].resize(len), p[rs].resize(len), p[ls].dft(0, len);
      p[rs].dft(0, len), p[i].resize(len), p[i].mul(p[ls], p[rs], 0, len);
  ...
  std::vector<mod_t> multi_eval(const poly &f) const {
    ...
    if (m > nn)
      ...
      std::copy(q.begin(), q.end(), GA1::bf + nn - m),
          std::fill(GA1::bf, GA1::bf + nn - m, Z);
    for (int k = nn; k >>= 1;)
      for (int i = 0, k2 = k << 1, o = nn / k; i < nn; i += k2, o += 2) {
        GNA1::bf.set(GA1::bf + i, 0, k2), GNA1::bf.dft(0, k2);
        GNA2::bf.mul(GNA1::bf, p[o | 1], 0, k2), GNA2::bf.idft(0, k2);
        GNA1::bf.mul(p[o], 0, k2), GNA1::bf.idft(0, k2);
        GNA2::bf.get(GA1::bf + i - k, k, k2), GNA1::bf.get(GA1::bf + i, k, k2);
      }
  ...
  poly interpolate(const std::vector<mod_t> &ys) {
    ...
    std::fill(GA1::bf + n, GA1::bf + nn, Z);
    for (int k = 1; k < nn; k <<= 1)
      for (int i = 0, o = nn / k, k2 = k << 1; i < n; i += k2, o += 2) {
        GNA1::bf.set(GA1::bf + i, 0, k), GNA1::bf.zeros(k, k2);
        GNA2::bf.set(GA1::bf + i + k, 0, k), GNA2::bf.zeros(k, k2);
        GNA1::bf.dft(0, k2), GNA2::bf.dft(0, k2), GNA1::bf.mul(p[o | 1], 0, k2);
        GNA2::bf.mul(p[o], 0, k2), GNA1::bf.add(GNA2::bf, 0, k2);
