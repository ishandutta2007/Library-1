...
template <std::size_t _Nm, class mod_t> void div_at_ntt(std::vector<mod_t> &p, std::vector<mod_t> &q, std::uint64_t &k) {
...
template <std::size_t _Nm, class mod_t> void div_at_ntt_fast(std::vector<mod_t> &p, std::vector<mod_t> &q, std::uint64_t &k) {
 static_assert(is_nttfriend<mod_t, _Nm>());
 using ntt= NTT<mod_t>;
...
#define __FPS_DIVAT(Vec) \
 template <class mod_t, std::size_t _Nm= 1 << 22> mod_t div_at(Vec p, Vec q, std::uint64_t k) { \
  ...
  if (k >= m) { \
   if constexpr (is_nttfriend<mod_t, _Nm>()) m <= 44 ? div_at_na(p, q, k) : div_at_ntt_fast<_Nm>(p, q, k); \
   else m <= 340 ? div_at_na(p, q, k) : div_at_ntt<_Nm>(p, q, k); \
  } \
...
// return a[k]
template <class mod_t, std::size_t _Nm= 1 << 22> mod_t linear_recurrence(std::vector<mod_t> c, std::vector<mod_t> a, std::uint64_t k) {
