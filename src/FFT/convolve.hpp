...
template <class mod_t, std::size_t LIM= 1 << 22> std::vector<mod_t> convolve(const std::vector<mod_t> &p, const std::vector<mod_t> &q) {
  mod_t *pp= GlobalArray<mod_t, LIM, 0>::bf, *qq= GlobalArray<mod_t, LIM, 1>::bf, *rr= GlobalArray<mod_t, LIM, 2>::bf;
  static constexpr int t= nttarr_cat<mod_t, LIM>, TH= (int[]){70, 30, 70, 100, 135, 150}[t];
  auto f= [](int l) -> int {
  ...
  if (std::min(n, m) < TH) {
    ...
    if (b || (l + fl < sz && sz <= (rl >> 3) * 5)) {
      ...
      auto gt1= GlobalNTTArray2D<mod_t, LIM2, 16, 1>::bf, gt2= GlobalNTTArray2D<mod_t, LIM2, 16, 2>::bf;
