...
// BEGIN CUT HERE
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> div(const std::vector<mod_t> &p, const std::vector<mod_t> &q) {
 ...
 const int n= p.size(), len= pw2(n), R= len < TH2 ? 8 : 16;
 ...
 if (l < A * __builtin_ctz(len) + B || TH >= n) {
  for (int i= 0; i < n; GAp::bf[i++]*= iv0)
   for (int j= std::min(i + 1, l); --j;) GAp::bf[i]-= GAp::bf[i - j] * GAq::bf[j];
  return std::vector<mod_t>(GAp::bf, GAp::bf + n);
 }
 int m= len, i= 0;
 while (m > TH) m>>= lnR;
 for (std::copy_n(GAp::bf, m, GAr::bf); i < m; GAr::bf[i++]*= iv0)
  for (int j= std::min(i + 1, l); --j;) GAr::bf[i]-= GAr::bf[i - j] * GAq::bf[j];
 ...
 for (GA::bf[0]= iv0, i= 1; m < n; i= m, m<<= lnR) {
  ...
  for (GNA2D2::bf[0].dft(0, m2); k < ed; mm= std::min(m, n - m * ++k)) {
   ...
   for (j= k; j--;) GNA3::bf.mul(GNA2D1::bf[j], GNA2D2::bf[k - j], 0, m2), GNA2::bf.add(GNA3::bf, 0, m2);
