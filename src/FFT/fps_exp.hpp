...
template <class mod_t> std::vector<mod_t> deriv(const std::vector<mod_t> &p) {
 std::vector<mod_t> ret(p.size() - 1);
 for (int i= p.size(); --i;) ret[i - 1]= p[i] * i;
 return ret;
}
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> integ(const std::vector<mod_t> &p) {
 std::vector<mod_t> ret(p.size() + 1, mod_t(0));
 for (int i= p.size(); i; i--) ret[i]= p[i - 1] * get_inv<mod_t, _Nm>(i);
 return ret;
}
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> log(const std::vector<mod_t> &p) {
 assert(p[0] == mod_t(1));
 return integ<mod_t, _Nm>(div<mod_t, _Nm>(deriv(p), p));
}
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> exp(const std::vector<mod_t> &p) {
 ...
 for (int r= m, d= 0, R, k, i; r > TH; d+= k) {
  ...
  for (i= std::min(R - 1, (n - 1) / r); i--;) GNA2D1::bf[i].set(GAdp::bf + i * r - d, d, d + k), GNA2D1::bf[i].dft(d, d + k);
 }
  ...
  if (int i= l | (!l), ed= std::min(r, n), j; r - l > TH) {
   ...
   for (i= 0, ed= std::min(R, (n - l + len - 1) / len);; i++) {
    if (mod_t *ret= GAr::bf + l + i * len, *bf= GA::bf + d + len; i) {
     for (GNA1::bf.zeros(d, d + k), j= i; j--;) GNA2::bf.mul(GNA2D2::bf[j], GNA2D1::bf[i - j - 1], d, d + k), GNA1::bf.add(GNA2::bf, d, d + k);
...
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> pow(const std::vector<mod_t> &p, std::uint64_t k) {
...
using ntt_internal::deriv, ntt_internal::integ, ntt_internal::log, ntt_internal::exp, ntt_internal::pow;