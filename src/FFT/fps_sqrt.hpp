...
// BEGIN CUT HERE
template <class mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> sqrt(const std::vector<mod_t> &p) {
 ...
 for (GA3::bf[0]= mod_t(1) / bf[0], i= 1; m < nn; i= m, m<<= lnR) {
  ...
  for (mod_t *bfk= bf, *pbfk= pbf; k < ed; mm= std::min(m, nn - m * ++k)) {
   ...
   for (GNA2::bf.zeros(0, m2), j= (k - 1) >> 1; j--;) GNA3::bf.add(GNA2D1::bf[j + 1], GNA2D1::bf[j], 0, m), GNA3::bf.dif(GNA2D1::bf[j + 1], GNA2D1::bf[j], m, m2), GNA3::bf.mul(GNA2D1::bf[k - 1 - j], 0, m2), GNA2::bf.add(GNA3::bf, 0, m2);
   if (j= k >> 1; !(k & 1)) GNA3::bf.mul(GNA2D1::bf[j], GNA2D1::bf[k - 1 - j], 0, m2), GNA2::bf.add(GNA3::bf, 0, m), GNA2::bf.dif(GNA3::bf, m, m2);
