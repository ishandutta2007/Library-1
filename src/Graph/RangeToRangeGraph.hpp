...
public:
 ...
 void add(int s, int t, T w= 0) { graph.add_edge(s, t), weight.emplace_back(w); }
 // [s_l, s_r) -> t
 ...
 // [s_l, s_r) -> [t_l, t_r)
 void add_from_range_to_range(int s_l, int s_r, int t_l, int t_r, T w= 0) { add_from_range(s_l, s_r, graph.n, w), add_to_range(graph.n, t_l, t_r, 0), ++graph.n; }
};