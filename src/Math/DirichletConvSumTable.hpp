...
template <class T> struct DirichletConvSumTable {
 ...
 DirichletConvSumTable(std::uint64_t n_, const std::vector<T> &x_, const std::vector<T> &X_): N(n_), x(x_), X(X_) { assert(N < std::uint64_t(x.size()) * X.size()); }
 DirichletConvSumTable(std::uint64_t n_, std::size_t k_): N(n_), x(k_ + 1, 0), X((n_ - 1 + k_) / k_ + 1, 0) {}
 template <class F> DirichletConvSumTable(std::uint64_t n_, std::size_t k_, const F &sum): N(n_), x(k_ + 1), X((n_ - 1 + k_) / k_ + 1) {
 ...
 DirichletConvSumTable operator*(const DirichletConvSumTable &r) const {
  ...
  for (std::size_t l= L, m, i; l; C[l--]-= A(m) * B(m))
   for (i= m= std::sqrt(n= N / l); i; i--) C[l]+= x[i] * B(n / i) + r.x[i] * A(n / i);
  return DirichletConvSumTable<T>(N, c, C);
 }
 DirichletConvSumTable &operator*=(const DirichletConvSumTable &r) { return *this= *this * r; }
 DirichletConvSumTable operator/(const DirichletConvSumTable &r) const { return DirichletConvSumTable(*this)/= r; }
 DirichletConvSumTable &operator/=(const DirichletConvSumTable &r) {
  ...
  for (std::size_t l= L, m; l; X[l--]/= r.x[1])
   for (m= std::sqrt(n= N / l), X[l]+= A(m) * B(m) - x[1] * A(n); m > 1;) X[l]-= r.x[m] * B(n / m) + x[m] * A(n / m), m--;
  return *this;
 }
...
T dirichlet_mul_sum(const DirichletConvSumTable<T> &a, const DirichletConvSumTable<T> &b) {
...
auto SemiRelaxed(const DirichletConvSumTable<mod_t> &F, mod_t s, mod_t t) {
 ...
 for (std::size_t j= 1, i, ed; j <= K; j++)
  for (g[j]/= F.x[1] - t, i= 2, ed= K / j; i <= ed; i++) g[i * j]-= F.x[i] * g[j];
 ...
 for (std::size_t l= L, m; l; (G[l--]+= s)/= F.x[1] - t)
  for (m= std::sqrt(n= F.N / l), G[l]= A(m) * B(m) - g[1] * A(n); m > 1;) G[l]-= F.x[m] * B(n / m) + g[m] * A(n / m), m--;
