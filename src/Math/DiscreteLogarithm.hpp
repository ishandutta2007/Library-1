...
public:
 DiscreteLogarithm(const F &mapping, const G &op, const H &hash, int64_t lim= 1ll << 50): mapping(mapping), op(op), hash(hash), lim(lim) { static_assert(std::is_convertible_v<std::invoke_result_t<H, T>, int>); }
 int64_t operator()(const E &x, T s, const T &t, int64_t N= -1) const {
  ...
  T s1= t;
  for (int i= 0; i < m; ++i) ++os[so[i]= hash(val[i]= s1= mapping(s1, x)) & mask];
  ...
  for (int64_t n= 0;; s= s1) {
   for (int a= hash(s1= mapping(s, y)) & mask, j= os[a]; j < os[a + 1]; ++j) {
    if (s1 == vs[j]) {
