...
public:
 // O(n 2^n)
 template <typename T> static inline void subset_sum(std::vector<T> &f) { SUBSET_REP(S, U, f.size()) f[S]+= f[U]; }
 // O(n 2^n)
 template <typename T> static inline void subset_sum_inv(std::vector<T> &f) { SUBSET_REP(S, U, f.size()) f[S]-= f[U]; }
 // O(n^2 2^n)
 template <class T> static inline std::vector<T> convolve(const std::vector<T> &f, const std::vector<T> &g) {
 ...
 // f(S) = φ_S ( Σ_{∅≠T⊊S & (T<(S/T) as binary numbers) } f(T)f(S/T) )   O(n^2 2^n)
 template <class T, class F> static inline std::vector<T> self_relaxed_convolve(int n, const F &phi) {
 ...
 template <class T, class EGF> static inline std::vector<T> composite(const std::vector<T> &f, const EGF &F) {
 ...
 // exp(f) : "f[∅] = 0" is required.  O(n^2 2^n)
 template <class T> static inline std::vector<T> exp(const std::vector<T> &f) {
  const int sz= f.size();
 ...
 template <class T> static inline std::vector<T> log(const std::vector<T> &f) {
  const int sz= f.size();
 ...
 // f^k   O(n^2 2^n)
 template <class T> static inline std::vector<T> pow(std::vector<T> f, uint64_t k) {
  const int sz= f.size(), n= __builtin_ctz(sz);
  assert(sz == 1 << n);
 ...
 template <class T> static inline std::vector<T> egf(std::vector<T> f) {
 ...
 template <class T> static inline std::vector<T> egf(const std::vector<T> &f, std::vector<T> g) {
  static constexpr int M= 1 << 11;
