  ...
  if (s[0][p] != s[0][p - 1]) {
   ...
   for (T pw= 1; k < d; k++, pw*= p)
    if (!k || poly[k] != T(0)) {
     ...
     for (int i= sqrtN; (std::uint64_t)i >= q; i--) s[k][i]-= (s[k][double(i) / p] - tk) * pw;
    }
  }
 for (int n= 1; n <= sqrtN; n++)
...
auto prime_count_table(std::uint64_t N) { return polynomial_prime_sum_table<std::uint64_t>(N, {1}); }
std::uint64_t prime_count(std::uint64_t N) { return std::get<2>(prime_count_table(N))[1]; }
template <class T> T polynomial_prime_sum(std::uint64_t N, const std::vector<T> &poly) { return std::get<2>(polynomial_prime_sum_table<T>(N, poly))[1]; }
template <class T, class F> T additive_sum(std::uint64_t N, F f, std::vector<T> poly) {
 const std::uint64_t sqrtN= std::sqrt(N);
 auto [primes, s, l]= polynomial_prime_sum_table<T>(N, poly);
 T ret= l[1];
 for (std::uint64_t d= 2, nN= N / d, nd; nN; nN= N / (d= nd)) ret+= (nN > sqrtN ? l[d] : s[nN]) * ((nd= N / nN + 1) - d);
 for (std::uint64_t p: primes)
  for (std::uint64_t pw= p * p, e= 2; pw <= N; e++, pw*= p) ret+= (f(p, e) - f(p, e - 1)) * (N / pw);
 return ret;
}
template <class T= __int128_t, class F> T multiplicative_sum(std::uint64_t N, const F &f, const std::vector<T> &poly) {
 const std::uint64_t sqrtN= std::sqrt(N);
 std::vector<int> primes;
 std::vector<T> s, l;
