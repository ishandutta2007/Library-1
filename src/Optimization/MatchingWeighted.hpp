...
template <class cost_t= long long> class MatchingWeighted {
 ...
 int findeven(int b, int v, int p= 0) {
  if (p= std::find(fwr[b].begin(), fwr[b].end(), v) - fwr[b].begin(); p & 1) std::reverse(fwr[b].begin() + 1, fwr[b].end()), p= fwr[b].size() - p;
  return p;
 }
  ...
  if (int u= rt[e.u], v= rt[e.v], bu= u, bv= v, x; isS[v] == 1) {
   ...
   for (int i, id= n + 1; bv; bv= rt[mt[bv]] ? rt[par[rt[mt[bv]]]] : 0)
    if (used[bv] == in) {
     ...
     for (int c: fwr[id]) {
      for (i= 1; i <= m; i++)
       if (!G[id][i].w || dist(G[c][i]) < dist(G[id][i])) G[id][i]= G[c][i], G[i][id]= G[i][c];
      for (i= 1; i <= n; i++)
  ...
  for (cost_t del= INF;; del= INF) {
    ...
    for (v= que.front(), que.pop(), i= 1; i <= n; i++)
     if (G[v][i].w && rt[i] != rt[v]) {
      if (!dist(G[v][i])) {
       ...
       if (!slk[rt[i]] || dist(G[v][rt[i]]) < dist(G[slk[rt[i]]][rt[i]])) slk[rt[i]]= v;
     }
   for (int i= n + 1; i <= m; i++)
   ...
   for (int b= n + 1, x, p, i, T, S; b <= m; b++)
    if (rt[b] == b && isS[b] == 2 && dual[b] == 0) {
     ...
     for (p= findeven(b, x), i= 0, x= fwr[b].size(); i < p; i+= 2) isS[S= fwr[b][i + 1]]= 1, isS[T= fwr[b][i]]= 2, par[T]= G[S][T].u, slk[S]= slk[T]= 0, push(S);
...
public:
 MatchingWeighted(int _n): n(_n), m(n), in(0), G(n * 2, std::vector<E>(n * 2)), mt(n * 2), rt(n * 2), used(n * 2), fwr(n * 2), blg(n * 2, std::vector<int>(n * 2)), dual(n * 2) {
