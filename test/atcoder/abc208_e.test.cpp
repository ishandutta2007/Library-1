...
public:
 ...
 vector<symbol_t> alphabet() const { return {1, 2, 3, 4, 5, 6, 7, 8, 9}; }
 inline state_t initial_state() const { return 1; }
 inline state_t transition(state_t s, const symbol_t &a, int) const { return min(s * a, K + 1); }
...
signed main() {
 ...
 auto dfa_le2= DFA_SymbolMap(dfa_le, dfa_pro.alphabet(), [](int x) { return x; });
