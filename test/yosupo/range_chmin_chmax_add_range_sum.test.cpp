...
using namespace std;
struct Mono {
  static constexpr long long INF = LLONG_MAX / 2.1;
  struct T {
  ...
  static long long min2(long long a, long long a2, long long b, long long b2) {
    return a == b ? min(a2, b2) : a2 <= b ? a2 : b2 <= a ? b2 : max(a, b);
  }
  static long long max2(long long a, long long a2, long long b, long long b2) {
  ...
  static void composition(E &pre, const E &suf) {
    if (auto tmpl = suf.lb - pre.ad; pre.ub <= tmpl)
      pre.ub = pre.lb = tmpl;
    else if (auto tmpu = suf.ub - pre.ad; tmpu <= pre.lb)
      pre.ub = pre.lb = tmpu;
    else
